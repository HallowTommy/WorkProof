...
private extension Color {
    init(wgHex: UInt, alpha: Double = 1.0) {
        self.init(
            .sRGB,
            red: Double((wgHex >> 16) & 0xFF) / 255,
            green: Double((wgHex >> 8) & 0xFF) / 255,
            blue: Double(wgHex & 0xFF) / 255,
            opacity: alpha
        )
    }
}
